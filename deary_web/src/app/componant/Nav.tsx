...
export default function Nav() {
  ...
  return (
    <nav className="fixed z-50 top-0 w-screen inset-x-0 h-[75px] bg-[#363636] font-mitr ">
      <div className=" min-[450px]:hidden flex items-center justify-between h-[75px]">
        <Link
          href="/"
        ...
        {/* nav เต็ม */}
        <div className="hidden sm:flex  items-center justify-center space-x-2 min-[321px]:space-x-8 mr-6 sm:mr-8 md:mr-10">
          <Link
            href="/year-total"
            className="hidden sm:block  text-white hover:scale-110 duration-100 text-[12px] sm:text-[14px] md:text-[16px] "
          >
            Diary{" "}
          </Link>
          ...
          {username !== "" ? (
            <>
              <div className="circle hidden sm:block ">{username.charAt(0).toUpperCase()}</div>
              <button
                onClick={() => handleSignOut()}
                className="hidden sm:block justify-center items-center text-white bg-[#DB0361] w-[100px] rounded-[10px] p-2 border-black border-2 mb-2 shadow-[7px_6px_black] transition ease-in-out delay-130 hover:-translate-y-1 hover:scale-105 hover:bg-[#C60458] duration-100"
                >
                Sign Out
              </button>
            </>
        ...
        {/* สามขีด */}
        <div className="flex sm:hidden items-center mr-6 sm:mr-8">
